...
Turns out microservices is just a relatively new **term**. The concept of microservices has actually been around for a while, it just never had an official name. Microservices is actually a subset of Service-oriented Archetecture (SOA). SOA, has actually been around for quite some time.
...
SOA, however has created a bad name for itself due to certian paterns such as "Enterprise Service Bus" which are now considered anti-pattern. Thus a term for a specific "style" of SOA has been created, which is now called "microservices". This brings the question, what exactly **is** a microservice

## What is a Microservice

A microservice is a SOA where each service has a couple restrainst impossed upon them.
...
This is in contridiction of the Enterprise Service Bus, where the bus itself often handles lots of choreography and tranformations. It is important that logic be built into the actual service that its for. 

2) **Hard boundries**. Microservices should always talk to eachother using a standard protocol, and never using functions/libraries. This protocol if often HTTP. Microservices should never allow other services to reach into their code without going through the defined contract and protocol.
...
**A database is not a contract**. If multiple services are sharing a database, that is not microservices.

These contraints must be kept in order to truly reap the benefits of microservices. To learn more about microservices,
feel free to read [Martin Fowlers post on it](https://www.martinfowler.com/articles/microservices.html), or read the book [Building Microsevices by Sam Newman](https://www.amazon.com/Building-Microservices-Designing-Fine-Grained-Systems/dp/1491950358)

## Pros and Cons of Microservices

Nothing is a silver bullet and that includes Microservices. Microservices are not for everyone. Deciding whether to use  microservices or not is about deciding where your priorities lie. The typicall comparison for most is Microservices vs Monolith.
A monolith is a single deployable app which contains all application code. There is only one code base, only one 
deployable, and one database (well, unless your sharding). A monolith is the exact opposite of a SOA archetecture, so it balances the cons of microservices very well. That being said, there might be some models that skirt the middle of the two.
...
When you have multiple code bases, your teams are less likely to step on eachothers toes.
...
3) **Enforced Boundries**. In single repo projects, boundries are often defined, but are very "theoretical". Breaking the boundries by having seperate code bases prevent people from creating code that crosses several boundries. It is just too easy to cross boundries in a single repo, and when people have deadlines, they are likely to cut corners.

4) **Autonomy**. Conways Law says that a software archetecture will eventually mirror the organization communication structure.
If you want "small blast zones" you have to create Automonous teams. It is too difficult to create automous teams when
everyone if working on a single repo. If you go with the monolith you end up getting a very hierarchical structure in your organization, as you need certian people/teams to coordinate everything.
...
In a single repo, rewrites are near impossible without freezing feature developement. 
Sometimes the entire app still needs to be rewritten or changed (perhaps your changing your database?). 
In these cases, you can change one service at a time, and not have to full freeze feature developement.

8) **Tech Stack agnostic**. Because services are broken up into hard boundries, this allows you to use different tech stacks for
...
In microservices however, each service could have its own tech stack; you could even run your own expirements as to which
tech stack is working better, etc. If you decide you need to move to a new stack acrossed services, you can do it one service at a time.

### Cons
1) **Performance**. Microsevices often have to call many other services. Other services have to go to other databases.
...
3) **Tools/Configuration Management**. That that there are more than one service to manage, tools are required to manage services and their configuraton. Many new tools will have to enter your stack to manage the full picture. You might want 
...
Tools and things will be required to manage the operations overhead.
...
6) **Service Discovery**. Now that there are many services, you need a way for the services to find/talk to eachother.

7) **Backwards Compatibility**. When you deploy everything together, you can easily change things such as an API. The new code calls the new API, and everything is great. In a microservice world, you cant control other services, you your API's must stay backwards compatible potentially forever. 
...
We believe that when you empower developers to make their own decisions on their own, they move fast and make better decisions. Sure, they might make bad decisions sometimes, but as long as we fail fast, then we can learn from it.

* **Frequent Deploys**: The best way to be successfull is to make many mistakes and to fail. *Ken Robinson* said: "`If you're not prepared to be wrong, you'll never come up with anything original.`" At Canopy, if we are going to innovate, we need to be able to fail many times. Ideally we recover from our failures as quickly as possible. In order to do that, we need to have short frequent deploys. Infrequent deploys will scare us into trying new things.
...
* **Operation Management**: Microservices adds a very big overhead to operations. This could require an entire operations team. Instead, Canopy has decided to help mitigate this issue by building a culture of "You Build It - You Run It". Developers are essentually responsible for handling the operations of their services. 

* **Tools/Configuration Mangement**: Lots of tools and things are required to handle the many services. This can be mitigated by hiring a DevOps team. A team responsible for helping manage all the configuration and services by providing tools to help get various tasks done.


## Conclusion
In Summary, Canopy has chosen microservices despite the upfront cost and challenges in order to
help foster a culture of autonomy and fast-failing. As Canopy is growing really fast, the microservices seperation of responsibilities should help us scale as an organization without slowing our rate of innovation. 
